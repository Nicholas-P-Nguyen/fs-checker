...
int open_test_file() {
  char filename[32];
  assert(0 < snprintf(filename, sizeof(filename), "./tester/tests/%d.img", test_counter));
  test_counter++;
...
void make_test_file(bool should_succeed, char *desc, char *error, 
  ...
  assert(0 < snprintf(filename, sizeof(filename), "./tester/tests/%d.desc", test_counter));
  make_file_with(filename, desc);

  // make the test #.run file
  assert(0 < snprintf(filename, sizeof(filename), "./tester/tests/%d.run", test_counter));
  char run_data[32];
  assert(0 < snprintf(run_data, sizeof(run_data), "./xcheck ./tests/%d.img\n", test_counter));
  make_file_with(filename, run_data);

  // make the test #.rc file
  assert(0 < snprintf(filename, sizeof(filename), "./tester/tests/%d.rc", test_counter));
  if (should_succeed) {
  ...
  assert(0 < snprintf(filename, sizeof(filename), "./tester/tests/%d.err", test_counter));
  make_file_with(filename, error);

  // make the test #.out file
  assert(0 < snprintf(filename, sizeof(filename), "./tester/tests/%d.out", test_counter));
